...
const TKINFO_BYTES: &[u8] = &[
    ...
    0x30, 0x20, 0x2d, 0x6d, 0x20, 0x36, 0x34, 0x20, 0x00, 0x00, 0x00, 0x00,
];

/// .note.nv.cuinfo desc: 12 bytes matching working nvcc 12.8 cubins.
const CUVER_DESC: &[u8] = &[
    0x02, 0x00, 0x78, 0x00, 0x83, 0x00, 0x00, 0x00,
];
...
const CAPMERC_EXIT_STUB: &[u8] = &[
    0x0c, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0xc0,
    0x0a, 0x00, 0x00, 0x00, 0x4c, 0x01, 0x00, 0x00,
...
const MERC_DEBUG_FRAME: &[u8] = &[
    ...
    0xff, 0xff, 0xff, 0xff, 0x0f, 0x0c, 0x81, 0x80,
    0x80, 0x28, 0x00, 0x08, 0xff, 0x81, 0x80, 0x28,
    0x08, 0x81, 0x80, 0x80, 0x28, 0x00, 0x00, 0x00,
    0xff, 0xff, 0xff, 0xff, 0x34, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0xb0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // PC_range = 0xb0 = CAPMERC_STUB_PADDED (nvcc 12.8)
    0x04, 0x10, 0x00, 0x00, 0x00, 0x04, 0xa0, 0x00, // DW_CFA_advance_loc4 0x10, then 0xa0 (EXIT at 0xa0)
    0x00, 0x00, 0x0c, 0x81, 0x80, 0x80, 0x28, 0x00,
    0x04, 0xf0, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
];
...
const MERC_HASH: &[u8] = &[
    0x8a, 0x9d, 0x22, 0xa4, 0xb1, 0x9d, 0x14, 0x6d,
    0x00, 0xb4, 0x2a, 0xf3, 0xf7, 0x58, 0x03, 0xa5,
    0x27, 0x2c, 0x21, 0x30, 0xc9, 0x1e, 0xc7, 0x8f,
    0x0f, 0x0c, 0x49, 0x6c, 0x0a, 0x2f, 0x00, 0x00,
];
...
pub fn generate_mercury_from_sass(code: &[u8], kernel_id: u32) -> Vec<u8> {
    ...
    // Based on working CAPMERC_EXIT_STUB format (verified on GEMV kernel).
    // Simple structure: header + base_record + const_record + stg_resource + writeback + tail
    // This format tells the driver "kernel uses global memory via descriptor" which
    // triggers UMD descriptor setup at c[0x0][0x358].
    let mut buf: Vec<u8> = Vec::new();

    // Header (12 bytes)
    buf.extend_from_slice(&kernel_id.to_le_bytes());
    buf.extend_from_slice(&0xC000_0001u32.to_le_bytes());
    buf.extend_from_slice(&(n_instr as u32).to_le_bytes());

    // Bitstream hint (4 bytes)
    let bits: u32 = if n_instr <= 5 { 0x0000_0002 }
        else if n_instr <= 50 { 0x0000_014c }
        else if n_instr <= 200 { 0x0000_014c }
        else { 0x0000_014c };
    buf.extend_from_slice(&bits.to_le_bytes());

    // Base record: descriptor setup
    buf.extend_from_slice(&[
        0x01, 0x0b, 0x04, 0x0a, 0xf8, 0x00, 0x04, 0x00,
        0x00, 0x00, 0x41, 0x00, 0x00, 0x04, 0x00, 0x00,
    ]);

    // Memory record: generic global memory access
    buf.extend_from_slice(&[
        0x01, 0x0b, 0x04, 0x0a, 0xf8, 0x00, 0x04, 0x00,
        0x00, 0x00, 0x01, 0x00, 0x01, 0x02, 0x00, 0x00,
    ]);

    // Constant bank record
    buf.extend_from_slice(&[
        0x01, 0x0b, 0x0e, 0x0a, 0xfa, 0x00, 0x05, 0x00,
        0x00, 0x00, 0x03, 0x01, 0x39, 0x04, 0x00, 0x00,
    ]);

    // STG resource descriptor (32 bytes)
    buf.extend_from_slice(&[
        0x02, 0x22, 0x0e, 0x06, 0xf8, 0x00, 0x52, 0x00,
        0x00, 0x00, 0x83, 0x00, 0x40, 0x00, 0x02, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    // Write-back descriptor (32 bytes)
    buf.extend_from_slice(&[
        0x02, 0x38, 0x0e, 0x32, 0xf8, 0x00, 0x40, 0x11,
        0x00, 0x00, 0x00, 0x00, 0x82, 0x00, 0x0a, 0x00,
        0x00, 0x02, 0x01, 0x40, 0x01, 0x00, 0x00, 0x00,
...
pub fn rebuild_cubin(template_bytes: &[u8], patches: &[CubinPatch<'_>]) -> Result<Vec<u8>> {
    ...
    for sec_idx in 0..cubin.text_sections.len() {
        ...
        let (patch_name, patch_code, patch_merc) = if let Some((name, code, merc)) = matched {
            ...
            (patches[0].0, patches[0].1.as_slice(), patches[0].2.as_deref())
        } else {
            continue;
        };
        ...
        let nop: [u8; 16] = [0x18,0x79,0x00,0x00,0x00,0x00,0x00,0x00,
        ...
        for i in (0..code.len().min(orig_text.len())).step_by(16) {
            if i + 16 > code.len() || i + 16 > orig_text.len() { break; }
...
fn detect_max_register(code: &[u8]) -> u32 {
    ...
    for i in (0..code.len()).step_by(16) {
        ...
        if rd < 128 && rd > max_reg { max_reg = rd; }
        let rs1 = code[i + 3] as u32;
        if rs1 < 128 && rs1 > max_reg { max_reg = rs1; }
        let rs2 = code[i + 4] as u32;
        if rs2 < 128 && rs2 > max_reg { max_reg = rs2; }
        if i + 9 <= code.len() {
            let rs3 = code[i + 8] as u32;
            if rs3 < 128 && rs3 > max_reg { max_reg = rs3; }
        }
    }
    max_reg
}
...
fn rename_kernel_in_elf(bytes: &mut [u8], old_name: &str, new_name: &str) -> Result<()> {
    ...
    while i + old_bytes.len() <= bytes.len() {
        if &bytes[i..i + old_bytes.len()] == old_bytes {
            ...
            let followed_ok = i + old_bytes.len() >= bytes.len()
                || bytes[i + old_bytes.len()] == 0;
...
impl StrTable {
    ...
    fn _find(&mut self, s: &str) -> u32 {
        ...
        while i + bytes.len() < self.data.len() {
            if &self.data[i..i + bytes.len()] == bytes
                && self.data[i + bytes.len()] == 0
            {
                return i as u32;
            }
            i += 1;
...
impl CubinBuilder {
    ...
    fn build_no_mercury(mut self, kernels: &[KernelEntry]) -> Result<Vec<u8>> {
        ...
        let max_shared: u64 = kernels.iter()
        ...
        for k in kernels { sn_funcs.push(self.strtab.add(&k.name)); }
        let mut sn_const0 = Vec::new();
        for k in kernels { sn_const0.push(self.strtab.add(&format!(".nv.constant0.{}", k.name))); }
        let mut sn_shared = Vec::new();
        for k in kernels { sn_shared.push(self.strtab.add(&format!(".nv.shared.{}", k.name))); }

        // Build EIATTR data
        let global_info_data: Vec<u8> = {
            use crate::eiattr::{EiRecord, EiFmt, NvInfoSection};
            ...
            NvInfoSection { name: ".nv.info".into(), records }.to_bytes()
        };
        ...
        for k in kernels {
            let min_size = 0x380 + k.meta.params.iter()
                .map(|p| (p.offset + p.size) as usize).max().unwrap_or(0);
            const0_data.push(vec![0u8; min_size.div_ceil(4) * 4]);
        }

        // Pad text to 128B alignment
        let nop: &[u8] = &[0x18,0x79,0x00,0x00,0x00,0x00,0x00,0x00,
                            0x00,0x00,0x00,0x00,0x00,0xc0,0x0f,0x00];
        let bra_self: &[u8] = &[0x47,0x79,0xfc,0x00,0xfc,0xff,0xff,0xff,
        ...
        for k in kernels {
            ...
            while code.len() < aligned {
                if code.len() + 16 == aligned { code.extend_from_slice(bra_self); }
                else { code.extend_from_slice(nop); }
            }
            text_data.push(code);
        }
        ...
        for ki in 0..n { sym!(0, 0, 0, 0x03, 0, text_k(ki)); }
        // GLOBAL: kernel function syms
        for ki in 0..n {
            sym!(sn_funcs[ki], 0u64, text_data[ki].len() as u64, 0x12, 0x10, text_k(ki));
        }
        ...
        sec!(shn_shstrtab, SHT_STRTAB, 0, self.shstrtab.data.clone(), 1, 0, 0, 0);
        sec!(shn_strtab, SHT_STRTAB, 0, self.strtab.data.clone(), 1, 0, 0, 0);
        sec!(shn_symtab, SHT_SYMTAB, 0, symtab, 8, IDX_STRTAB as u32, first_global, 24);
        sec!(shn_tkinfo, SHT_NOTE, SHF_NV_TKINFO, TKINFO_BYTES.to_vec(), 4, 0, 0, 0);
        sec!(shn_cuver, SHT_NOTE, SHF_NV_CUVER, build_cuver_note(), 4,
             IDX_TKINFO as u32, idx_compat as u32, 0);
        sec!(shn_nv_info, SHT_CUDA_INFO, 0, global_info_data, 4, IDX_SYMTAB as u32, 0, 0);
        sec!(shn_compat, SHT_CUDA_COMPAT, 0, NV_COMPAT.to_vec(), 4, 0, 0, 0);
        for ki in 0..n {
            sec!(shn_nv_info_k[ki], SHT_CUDA_INFO, SHF_INFO_LINK,
                 per_kernel_info[ki].clone(), 4, IDX_SYMTAB as u32, text_k(ki) as u32, 0);
        }
        for ki in 0..n {
            sec!(shn_text_k[ki], SHT_PROGBITS, SHF_ALLOC | SHF_EXECINSTR,
                 text_data[ki].clone(), 128, IDX_SYMTAB as u32, func_sym_idx(ki) as u32, 0);
        }
        // .nv.shared.reserved.0 — always 64 bytes minimum
        sections.push((shn_shared, SHT_NOBITS, SHF_WRITE | SHF_ALLOC,
                       vec![], 1, 0, 0, 0));
        ...
        for ki in 0..n {
            ...
            sections.push((shn_shared_k[ki], SHT_NOBITS, flags,
                           vec![], 4, 0, text_k(ki) as u32, 0));
        }
        for ki in 0..n {
            sec!(shn_const0_k[ki], SHT_PROGBITS, SHF_ALLOC | SHF_INFO_LINK,
                 const0_data[ki].clone(), 4, 0, text_k(ki) as u32, 0);
        }
        ...
        if !offset.is_multiple_of(4) { offset += 4 - (offset % 4); }
        let mut sec_offsets = Vec::new();
        for (i, (_, sh_type, _, data, align, _, _, _)) in sections.iter().enumerate() {
            if i == 0 { sec_offsets.push(0); continue; } // NULL section at offset 0
            if *sh_type == SHT_NOBITS { sec_offsets.push(offset); continue; }
            if *align > 1 && !offset.is_multiple_of(*align) { offset += *align - (offset % *align); }
            sec_offsets.push(offset);
            offset += data.len() as u64;
        }
        if !offset.is_multiple_of(8) { offset += 8 - (offset % 8); }
        ...
        for (i, (sh_name, sh_type, sh_flags, data, sh_align, sh_link, sh_info, sh_entsize)) in sections.iter().enumerate() {
        ...
        let mut phi = 0usize;
        let write_ph = |buf: &mut Vec<u8>, phi: usize, p_type: u32, p_flags: u32,
                         p_offset: u64, p_filesz: u64, p_memsz: u64, p_align: u64| {
        ...
        write_ph(&mut buf, phi, PT_PHDR, PF_R,
                 ph_offset, ph_table_size, ph_table_size, 8);
        phi += 1;
        ...
        for ki in 0..n {
            ...
            write_ph(&mut buf, phi, PT_LOAD, PF_R | PF_X,
                     t_off, t_sz, t_sz, 8);
            phi += 1;
        }
        ...
        write_ph(&mut buf, phi, PT_LOAD, PF_R,
                 notes_start, notes_end - notes_start, notes_end - notes_start, 8);
        phi += 1;

        // PT_LOAD RW for shared memory (NOBITS)
        {
            let shared_off = sec_offsets[idx_shared];
            write_ph(&mut buf, phi, PT_LOAD, PF_R | PF_W,
                     shared_off, 0, max_shared, 8);
            phi += 1;
        }
    ...
    fn build(mut self, kernels: &[KernelEntry]) -> Result<Vec<u8>> {
        ...
        for ki in 0..n {
            emit_sym(&mut symtab, stn_text_k[ki], STB_LOCAL, STT_SECTION, STV_DEFAULT,
                     text_k(ki) as u16, 0, 0);
        }
        // [N+1] .nv.reservedSmem.offset0 WEAK OBJECT abs
        emit_sym(&mut symtab, stn_reserved_smem, STB_WEAK, STT_OBJECT, STV_DEFAULT,
                 0, max_shared, 4);
        // [N+2] __nv_reservedSMEM_offset_0_alias WEAK NOTYPE shared
        emit_sym(&mut symtab, stn_smem_alias, STB_WEAK, STT_NOTYPE, 0xa0,
                 idx_shared as u16, max_shared, 0);
        // [N+3] .debug_frame section sym
        emit_sym(&mut symtab, stn_debug_frame, STB_LOCAL, STT_SECTION, STV_DEFAULT,
                 IDX_DBG as u16, 0, 0);
        // [N+4] .nv.callgraph section sym
        emit_sym(&mut symtab, stn_callgraph, STB_LOCAL, STT_SECTION, STV_DEFAULT,
                 idx_cg as u16, 0, 0);
        // [N+5+ki] kernel function syms (GLOBAL, st_other=0x10 per nvcc SM120)
        for ki in 0..n {
            let code_size = kernels[ki].code.len() as u64;
            emit_sym(&mut symtab, stn_func_k[ki], STB_GLOBAL, STT_FUNC, STV_HIDDEN,
                     text_k(ki) as u16, 0, code_size);
        }
        // [2N+5+ki] .nv.constant0.Ki section syms
        for ki in 0..n {
            emit_sym(&mut symtab, stn_const0_k[ki], STB_LOCAL, STT_SECTION, STV_DEFAULT,
                     const0_k(ki) as u16, 0, 0);
        }
        ...
        for ki in 0..n {
            emit_sym(&mut symtab, stn_shared_k[ki], STB_LOCAL, STT_SECTION, STV_DEFAULT,
                     shared_k(ki) as u16, 0, 0);
        }
        ...
        for ki in 0..n {
            emit_sym(&mut merc_symtab, stn_capmerc_k[ki], STB_LOCAL, STT_SECTION, STV_DEFAULT,
                     capmerc_k(ki) as u16, 0, 0);
        }
        // [N+1] .nv.reservedSmem.offset0 WEAK OBJECT abs
        emit_sym(&mut merc_symtab, stn_reserved_smem, STB_WEAK, STT_OBJECT, STV_DEFAULT,
                 0, max_shared, 4);
        // [N+2] __nv_reservedSMEM_offset_0_alias WEAK NOTYPE merc_shared
        emit_sym(&mut merc_symtab, stn_smem_alias, STB_WEAK, STT_NOTYPE, 0xa0,
                 idx_merc_shared as u16, max_shared, 0);
        // [N+3] .nv.merc.debug_frame section sym
        emit_sym(&mut merc_symtab, stn_merc_debug, STB_LOCAL, STT_SECTION, STV_DEFAULT,
                 idx_merc_dbg as u16, 0, 0);
        // [N+4] .nv.callgraph section sym (references main callgraph)
        emit_sym(&mut merc_symtab, stn_callgraph, STB_LOCAL, STT_SECTION, STV_DEFAULT,
                 idx_cg as u16, 0, 0);
        // [N+5+ki] Mercury kernel function syms (GLOBAL, STV_HIDDEN)
        for ki in 0..n {
            let stub_sz = kernels[ki].mercury_stub.as_ref()
                .map(|s| s.len() as u64)
                .unwrap_or(CAPMERC_STUB_PADDED);
            emit_sym(&mut merc_symtab, stn_func_k[ki], STB_GLOBAL, STT_FUNC, STV_HIDDEN,
                     capmerc_k(ki) as u16, 0, stub_sz);
        }
        ...
        let merc_global_info: Vec<u8> = {
            ...
            NvInfoSection { name: ".nv.merc.nv.info".into(), records }.to_bytes()
        };
        ...
        sec!(shn_tkinfo,  SHT_NOTE,     SHF_NV_TKINFO,
             TKINFO_BYTES.to_vec(), 4, 0, 0, 0);  // 5
        // 6: .note.nv.cuinfo
        let cuver_data = build_cuver_note();
        sec!(shn_cuver,   SHT_NOTE,     SHF_NV_CUVER,
             cuver_data, 4,
             IDX_TKINFO as u32,  // sh_link → .note.nv.tkinfo
             idx_compat as u32,  // sh_info → .nv.compat
             0);
        // 7: .nv.info
        sec!(shn_nv_info, SHT_CUDA_INFO, 0,
             global_info_data.clone(), 4,
             IDX_SYMTAB as u32, 0, 0);

        // Per-kernel .nv.info.K (8..8+N)
        for ki in 0..n {
            sec!(shn_nv_info_k[ki], SHT_CUDA_INFO, SHF_INFO_LINK,
                 per_kernel_info[ki].clone(), 4,
                 IDX_SYMTAB as u32,
                 text_k(ki) as u32, 0);
        }

        // .nv.compat, .nv.callgraph, .rela.debug_frame
        sec!(shn_compat, SHT_CUDA_COMPAT, 0,
             NV_COMPAT.to_vec(), 4, 0, 0, 0);
        sec!(shn_cg,     SHT_CUDA_CALLGRAPH, 0,
             NV_CALLGRAPH.to_vec(), 4,
             IDX_SYMTAB as u32, 0, 8);
        sec!(shn_rela_dbg, SHT_RELA,     SHF_INFO_LINK,
             vec![], 8,
             IDX_SYMTAB as u32, IDX_DBG as u32, 24);

        // Per-kernel .text.K
        for ki in 0..n {
            sec!(shn_text_k[ki], SHT_PROGBITS,
                 SHF_ALLOC | SHF_EXECINSTR,
                 kernels[ki].code.clone(), 128,
                 IDX_SYMTAB as u32,
                 func_sym_idx(ki) as u32, 0);
        }

        // .nv.shared.reserved.0 (NOBITS, always 64 bytes minimum)
        sec!(shn_shared, SHT_NOBITS, SHF_WRITE | SHF_ALLOC,
             vec![], 1, 0, 0, 0, nobits(0x40));
        ...
        for ki in 0..n {
            ...
            if sh_size > 0 {
                secs.push((shn_shared_k[ki], SHT_NOBITS,
                           SHF_WRITE | SHF_ALLOC | SHF_INFO_LINK,
                           Vec::new(), 4,
                           0, text_k(ki) as u32, 0,
                           Some(sh_size)));
            } else {
        ...
        // Per-kernel .nv.constant0.K
        for ki in 0..n {
            sec!(shn_const0_k[ki], SHT_PROGBITS,
                 SHF_ALLOC | SHF_INFO_LINK,
                 const0_data[ki].clone(), 4,
                 0, text_k(ki) as u32, 0);
        }
        ...
        for ki in 0..n {
            ...
            sec!(shn_capmerc_k[ki], SHT_MERC_CAPMERC,
                 SHF_MERC,
                 stub.to_vec(), 16,
        ...
        // .nv.merc.debug_frame
        sec!(shn_merc_dbg, SHT_PROGBITS, SHF_MERC,
             MERC_DEBUG_FRAME.to_vec(), 1, 0, 0, 0);
        // .nv.merc.nv.info
        sec!(shn_merc_info, SHT_MERC_INFO, SHF_MERC,
             merc_global_info, 4,
             idx_merc_symtab as u32, 0, 0);

        // Per-kernel .nv.merc.nv.info.K
        for ki in 0..n {
            sec!(shn_merc_info_k[ki], SHT_MERC_INFO, SHF_MERC_LINK,
                 merc_per_kernel_info[ki].clone(), 4,
                 idx_merc_symtab as u32,
                 capmerc_k(ki) as u32, 0);
        }

        // .nv.merc.rela.debug_frame
        sec!(shn_merc_rela, SHT_MERC_RELA, SHF_MERC_LINK,
             merc_rela_data, 8,
             idx_merc_symtab as u32,
             idx_merc_dbg as u32, 24);
        // .nv.merc.nv.shared.reserved.0 (NOBITS size=0)
        sec!(shn_merc_sh, SHT_MERC_RESERVED_SH, SHF_MERC | SHF_WRITE | SHF_ALLOC,
             vec![], 1, 0, 0, 0);
        // .nv.merc.symtab
        sec!(shn_merc_symt, SHT_MERC_SYMTAB, SHF_MERC,
             merc_symtab, 8,
             IDX_STRTAB as u32,      // sh_link = .strtab (regular)
             merc_sym_first_global as u32, 24);

        assert_eq!(secs.len() + 4, total_sections,
                   "section count mismatch: {} + 4 != {}", secs.len(), total_sections);
        ...
        // .shstrtab
        write_shdr(&mut out, shn_shstrtab, SHT_STRTAB, 0, 0,
                   sec_off[IDX_SHSTR], sec_size[IDX_SHSTR], 0, 0, 1, 0);
        // .strtab
        write_shdr(&mut out, shn_strtab, SHT_STRTAB, 0, 0,
                   sec_off[IDX_STRTAB], sec_size[IDX_STRTAB], 0, 0, 1, 0);
        // .symtab
        write_shdr(&mut out, shn_symtab, SHT_SYMTAB, 0, 0,
                   sec_off[IDX_SYMTAB], sec_size[IDX_SYMTAB],
                   IDX_STRTAB as u32, reg_sym_first_global as u32, 8, 24);
        // Content sections
        for (si, spec) in secs.iter().enumerate() {
            let abs = si + 4;
            let (name_off, ty, flags, _, align, link, info, entsize, _) = spec;
            write_shdr(&mut out, *name_off, *ty, *flags, 0,
                       sec_off[abs], sec_size[abs],
        ...
        // PT_LOAD RX (.text)
        write_phdr(&mut out, 0x1, 0x5, text_start, 0,
                   text_end - text_start, text_end - text_start, 8);
        // PT_LOAD RW (.nv.shared.reserved.0)
        let shared_file_off = sec_off[idx_shared]; // = text_end (NOBITS shares offset)
        write_phdr(&mut out, 0x1, 0x6, shared_file_off, 0,
                   shared_filesz, shared_memsz, 8);
        // PT_LOAD R (.nv.constant0)
        write_phdr(&mut out, 0x1, 0x4, const_start, 0,
                   const_end - const_start, const_end - const_start, 8);
...
fn build_merc_nv_info_k(meta: &KernelMeta, _merc_func_sym: u32) -> Vec<u8> {
    ...
    // CUDA_API_VERSION
    let api_ver = if meta.cuda_api_version != 0 { meta.cuda_api_version } else { 0x83 };
    ...
    records.push(EiRecord {
        attr: 0x005a, fmt: EiFmt::Sized,
        data: MERC_HASH.to_vec()
    });

    // MAX_THREADS (12 bytes)
    let mut d = vec![0u8; 12];
    d[8] = 0x00; d[9] = 0xf0; d[10] = 0x21; d[11] = 0x00;
...
use crate::eiattr::KernelMeta as KernelMeta;
...
impl KernelMeta {
    ...
    pub fn to_kernel_records_with_sym_and_const(
        ...
        let api_ver = if self.cuda_api_version != 0 { self.cuda_api_version } else { 0x83 };
        records.push(EiRecord {
            attr: 0x0037, fmt: EiFmt::Sized,
            data: api_ver.to_le_bytes().to_vec()
        });
        ...
        for param in self.params.iter().rev() {
            ...
            d[11] = 0x00;
            records.push(EiRecord { attr: 0x0017, fmt: EiFmt::Sized, data: d });
        }
        ...
        records.push(EiRecord { attr: 0x005f, fmt: EiFmt::Byte, data: vec![1, 1] });
        records.push(EiRecord { attr: 0x004a, fmt: EiFmt::Half, data: vec![0, 0] });
        ...
        NvInfoSection { name: format!(".nv.info.{}", self.name), records }
    }
}
...
fn write_elf_header_flags(buf: &mut [u8], e_shoff: u64, e_shnum: u16, e_shstrndx: u16,
                          e_phoff: u64, e_phnum: u16, e_flags: u32) {
...
#[allow(clippy::too_many_arguments)]
fn write_phdr(out: &mut Vec<u8>, p_type: u32, p_flags: u32,
              p_offset: u64, p_vaddr: u64,
...
#[allow(clippy::too_many_arguments)]
fn emit_sym(out: &mut Vec<u8>,
            st_name: u32, binding: u8, sym_type: u8, other: u8,
