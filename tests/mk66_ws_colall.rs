...
//! gets NO 41476e0a/4147760a mini — orig zeroes those lanes corpus-wide
//! (cusparse.318 find_colors*: 57 lanes, 0 records; A/B before the fix
//! 41476e0a orig 180 vs new 237, after: 180==180). The mk64b rule (76 iff
//! site 0x28 else 6e) applies ONLY to a bare WARPSYNC.ALL; our
//! .ALL.COLLECTIVE spelling must not be caught by the `.contains(".ALL")` filter —
...
fn mk66_colall_no_mini() {
    // 7 WARPSYNC.ALL.COLLECTIVE lanes (like find_colors in cusparse.318):
    // none gets a ws-mini, regardless of sites.
    let ins = vec![
    ...
    // COLLECTIVE.ALL is neither reg-form nor d1wc47 (mk59 fail-closed .ALL):
...
fn mk66_colall_meta_path() {
    ...
    // only a bare WARPSYNC.ALL (lane 2, site 0x30/16=3? no: site 0x30 is
    // lane 3, lane 2 is not a site) -> 6e; COLLECTIVE.ALL (lane 1) NOTHING.
...
fn mk66_scan_lines_mirror() {
    // mirror of mercury::mc_scan_lines: the same rule on the raw text.
    ...
    // COLLECTIVE.ALL cut out of ws_lanes; bare .ALL lane5 without a BAR after it
