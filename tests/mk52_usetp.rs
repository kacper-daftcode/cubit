...
//! 42104014 (EX pair) / 42254214 (ULEA carry-out). Rules from the sm_100 corpus
//! (merclab/mk52 c1..c26; bitmap: the firing lane loses its bit — outside this test,
//! harness-validated):
//!  * non-EX head (dst UPn) + EX tail (..., UPn) pair: class mini on the head lane
//!    (3406 when imm in head OR tail; 3614 o.w.) + 4014 when the tail has no imm,
//!  * non-EX chain (last operand !?UP#): mini by its own imm,
//!  * ULEA "URd, UPcout, .." -> 42254214; ULEA.HI(.X) recordless.
...
fn mk52_skan_par_i_lancuchow() {
    let ins = vec![
        ...
        // an EX alone without a known head -> nothing (fail-closed)
        ...
        // iamax: a chain without imm -> 3614 on lane 16
...
fn mk52_skan_guard_i_dst_neg() {
    // a guard on the head does not block; dst UPT/PT untracked
