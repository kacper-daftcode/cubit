//! mk74: new IADD3.X carry minis 421d0a06/421d0814 + ULOP3-fc-imm 42280c06.
//! Rules from the sm_100 corpus (merclab/mk74 c3 bitmap-attr, c8/c9 fit):
//! count-EXACT 5893/5893 l2 kernels (0a06 305/305, 0814 7233/7233,
//! 80c06 527/527 — the lut==0xfc gate; 0xc0/0x3c/0x0c recordless).
...
fn iadd3x_imm_is_0a06() {
    ...
    // imm forms ended (PT,PT) recordless (corpus fit: only (Pn,Pn))
    assert_eq!(
...
fn iadd3x_reg_is_0814() {
    ...
    // hseqr chase_bulge: a (PT,PT) tail without UR -> record (14/14 resid)
    assert_eq!(
        merc_iadd3x_carry("IADD3.X R0, PT, PT, R0, R15, RZ, PT, PT ;"),
        Some(0x14081d42)
    );
    // negations: tok5 != RZ; UR in operands; 7 tokens; IADD3 without .X
...
fn ulop3_fcimm_is_80c06() {
    ...
    // lut 0xc0/0x3c recordless; guard fail-closed; LOP3 (R-form) recordless
...
fn minis_pipeline() {
    ...
    assert!(x.contains(&(0, 0x14081d42)), "present: {x:?}");
    assert!(x.contains(&(1, 0x060c2842)), "present: {x:?}");
    assert!(!x.iter().any(|&(_, m)| m == 0x060a1d42), "tok5!=RZ cannot be 0a06: {x:?}");
    assert_eq!(x.len(), 2);
}
