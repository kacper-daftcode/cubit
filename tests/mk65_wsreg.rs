//! mk65 (2026-08-13): pakiet minis rodziny 4147 — reg-form WARPSYNC:
//! mini 4147780a (lane jest site'em EIATTR-0x28 z .merc_cgsites) vs
//! 4147700a (poza sites). Korpus EXACT obustronnie (merclab/mk65 c5/c7/c9:
//! 18932/18932 kerneli, 324 kern z 78, 310+14 z 70). Formy: czysty
//! `WARPSYNC R<n>` i `WARPSYNC.EXCLUSIVE R<n>` (16/16 EXCLUSIVE poza sites);
//! .ALL/.COLLECTIVE maja wlasne rodziny (mk64b/mk59). Naprawa poboczna:
//! BAR.ARV nie dostaje juz pelnego rekordu 01475a16 (nadprodukcja +64/kern
...
fn mk65_wsreg_variants() {
    let ins = vec![
        ...
        mk_ins(4, "WARPSYNC.ALL", "WARPSYNC.ALL ;"),            // nie reg-form
    ...
    assert_eq!(mc.ws_reg, vec![(2, 0x78u8), (3, 0x78), (5, 0x70), (6, 0x70)]
        .into_iter()
        .filter(|(l, _)| *l != 5) // lane 5 = COLLECTIVE: NIE jest reg-form
        .collect::<Vec<_>>());
    // ws_minis tylko dla .ALL
...
fn mk65_exec_pos_skip_arv() {
    ...
    // BAR.ARV na lane 1 wylaczone z bar_pos; zostaja 2 prawdziwe BAR.SYNC.
    assert_eq!(meta.merc_bar_pos, vec![2, 3]);
}

#[test]
fn mk65_wsreg_meta_paths() {
    // ten sam wzorzec co wyzej przez pelny sass: site 0x28 -> 0x78
