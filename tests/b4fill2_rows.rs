//! b4fill2: decoder+encoder coverage for the rt98
...
//! Provenance: the internal research tree (derivation logs + gate).
...
static GOLD: &[(u128, &str)] = &[
    ...
    // render_only pending 099 coverage)
    ...
    // ATOMG/REDG .EL.STRONG.GPU forms (REDG: era desc+PT golden byl
    // pomiarem MISROUTE x2 [census408: vendor x4 = plain [R.U32+UR(+imm)]];
    // ride F2-iter258 BUG-454 -- ATOMG desc stoi)
    (0x000f6400082ee10600000051ffda39a8u128, "@P3 ATOMG.E.ADD.EL.STRONG.GPU PT, R218, desc[UR6][RZ.64], R81"),
    (0x0009e4000820e11400000051d100498eu128, "@P4 REDG.E.ADD.EL.STRONG.GPU [R209.U32+UR20], R81"),
    (0x0009e4000aa0e1140a2400404200298eu128, "@P2 REDG.E.AND.EL.STRONG.GPU [R66.U32+UR20+0xa2400], R64"),
    (0x0009e4000b20e1140a2400171900798eu128, "REDG.E.OR.EL.STRONG.GPU [R25.U32+UR20+0xa2400], R23"),
...
fn b4fill2_render_only() {
    ...
    // full-file byte-exact gate (the internal research archive
    let t = t103a();
    let idx = DecodeIndex::build(&t);
    for &(word, golden) in &[
    ...
    // BUG-094: the four era words below were pinned (b4fill2) to the
    ...
    // raw-address uniform-indexed era forms (099 candidate), so decode is
    ...
    // canon does not cover yet (099; sm120.json decodes all three
    // vendor-exact today via its LDG/STG keys).
    // BUG-099: canon coverage for the raw-address uniform-indexed
