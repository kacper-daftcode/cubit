//! mk73: domkniecie resid 01290004 (keep-400 oo 92->0): (a) .reuse na
//! operandach (trsv/sphpr/sphpmv — nvdis l2 opuszcza suffix, frozen-sass
//! drukuje, uproszczony parse R<num> odrzucal), (b) pelny kod guarda b4 =
//! merc_guard_code (pred<<3|uni<<1|neg; brak/@P7 -> 0xf8) zamiast troj-stanu
...
fn xor_guard_full_code() {
    ...
    // @!P0 -> 0x01; oryginalne rekordy w mk73 c1 dokladnie takie.
    let x = scan_xor(
        ".entry t\n    .reg R0-R79\n    @!P4 LOP3.LUT R41, R4.reuse, R31.reuse, RZ, 0x3c, !PT ;\n    @P0 LOP3.LUT R75, R75, R74, RZ, 0x3c, !PT ;\n    @!P0 LOP3.LUT R47, R47, R46, RZ, 0x3c, !PT ;\n",
    );
    assert!(x.contains(&(0, 41, 4, 31, 0x21)), "jest: {x:?}");
    assert!(x.contains(&(1, 75, 75, 74, 0x00)), "jest: {x:?}");
    assert!(x.contains(&(2, 47, 47, 46, 0x01)), "jest: {x:?}");
}
...
fn guard_code_edge() {
    ...
    assert_eq!(merc_guard_code(g(7, false, false).as_ref()), 0xf8); // @P7 jak brak
